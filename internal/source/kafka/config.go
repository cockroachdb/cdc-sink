...
func (c *Config) Bind(f *pflag.FlagSet) {
	...
	// SASL
	f.StringVar(&c.caCert, "saslClientId", "", "client ID for OAuth authentication from a third-party provider")
...
func (c *Config) buildSaramaConfig(ctx context.Context) error {
	...
	if c.caCert != "" {
		if caPool, err := secure.GetCa(c.caCert); err != nil {
...
func (config *Config) newTokenProvider(ctx context.Context) (sarama.AccessTokenProvider, error) {
	...
	if config.saslGrantType != `` {
		endpointParams = url.Values{"grant_type": {config.saslGrantType}}
	}

	tokenURL, err := url.Parse(config.saslTokenURL)
	if err != nil {
	...
	cfg := clientcredentials.Config{
		ClientID:       config.saslClientID,
		ClientSecret:   config.saslClientSecret,
		TokenURL:       tokenURL.String(),
		Scopes:         config.saslScopes,
